...
double phys_coord(double r, double s, double p1, double p2, double p3) {
...
double gaussian(double x, double y, double mu_x, double mu_y, double sigma_x,
                double sigma_y) {

  double coef = 1.0 / (2.0 * M_PI * sigma_x * sigma_y);
  double exp_term = exp(-0.5 * ((pow((x - mu_x) / sigma_x, 2.0)) +
                                (pow((y - mu_y) / sigma_y, 2.0))));
